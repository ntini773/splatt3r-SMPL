...
# ---- ABLATION SWITCH ----
# Set USE_POINTNET=True  to use PointNet++ encoder
# Set USE_POINTNET=False to use the original GCN encoder
USE_POINTNET = True

if USE_POINTNET:
    from src.anatomical_prior.pointnet_encoder import AnatomicalPointNetEncoder as AnatomicalEncoder
else:
    from src.anatomical_prior.gcn_encoder import AnatomicalGCNEncoder as AnatomicalEncoder
...
class MAST3RAnatomicalRefinement(L.LightningModule):
    """
    Wraps a pretrained Splatt3R checkpoint and adds an anatomical prior
    encoder (PointNet++ or GCN) via Cross-Attention inside the GaussianHead.

    The prior encoder is held in self.pointnet_encoder regardless of which
    backbone is active, giving clean and consistent checkpoint keys.
    adj is forwarded through the call stack but silently ignored by PointNet++,
    so the GCN variant can be swapped back in without any further changes.
    """
    ...
    def __init__(self, config):
        ...
        # ------------------------------------------------------------------
        # 1. Load the full Splatt3R model via Lightning API
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # 2. Freeze Splatt3R backbone
        # ------------------------------------------------------------------
        self.encoder.requires_grad_(False)
        self.decoder.requires_grad_(False)

        # ------------------------------------------------------------------
        # 3. Selectively unfreeze DPT heads + new attention layers
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # 4. Initialise the prior encoder (PointNet++ or GCN)
        # ------------------------------------------------------------------
        if USE_POINTNET:
            self.pointnet_encoder = AnatomicalEncoder(
                in_channels=3,
                hidden_channels=getattr(config, 'pointnet_hidden', 128),
                out_channels=32,
                sa_npoints=tuple(getattr(config, 'pointnet_sa_npoints', [128, 32])),
                sa_radii=tuple(getattr(config, 'pointnet_sa_radii', [0.05, 0.20])),
                sa_nsamples=tuple(getattr(config, 'pointnet_sa_nsamples', [32, 64])),
            )
        else:
            self.pointnet_encoder = AnatomicalEncoder(
                in_channels=3, hidden_channels=128, out_channels=32
            )
    ...
    def forward(self, view1, view2, adj, anchors1, anchors2):
        ...
        # ---- prior encoder ----
        # PointNet++ ignores adj; GCN uses it. Both accept the same signature.
        prior1 = self.pointnet_encoder(anchors1, adj)   # [B, 512, 32]
        prior2 = self.pointnet_encoder(anchors2, adj)   # [B, 512, 32]
    ...
    def training_step(self, batch, batch_idx):
        ...
        adj = batch['smpl_adj'][0]          # still forwarded; PointNet++ silently ignores it
    ...
    def configure_optimizers(self):
        # Two param groups — same LR strategy as GCN variant:
        #   1. prior encoder + cross-attention: full LR (trained from scratch)
        #   2. Gaussian DPT heads:              0.1x LR  (pretrained, fine-tune gently)
        prior_and_attn_params = (
            list(self.pointnet_encoder.parameters()) +
        ...
        param_groups = [
            {
                'params': prior_and_attn_params,
                'lr': self.config.opt.lr,
                'name': 'pointnet_encoder_and_attention',
            },
            {
                'params': dpt_params,
                'lr': self.config.opt.lr * 0.1,
                'name': 'gaussian_dpt',
            },
        ]
...
def run_dist_experiment(config):
    ...
    dataset_root = getattr(config.data, 'root', '/ssd_scratch/adityapeketi/mvhumannet++_10demo')
    ...
    trainer = L.Trainer(
        ...
        callbacks=[
            L.pytorch.callbacks.ModelCheckpoint(
                dirpath='/ssd_scratch/adityapeketi/pointnet_checkpoints',
    ...
    if trainer.is_global_zero:
        ...
        with open(summary_path, 'w') as f:
            ...
            f.write(f"Encoder:           {'PointNet++' if USE_POINTNET else 'GCN'}\n")
            f.write(f"Best Checkpoint:   {best_model_path}\n")
            f.write(f"Best Loss Score:   {best_model_score}\n\n")
            f.write("Final Logged Metrics:\n")
            for k, v in trainer.logged_metrics.items():
                val = v.item() if hasattr(v, 'item') else v
                f.write(f"  {k}: {val}\n")

        print(f"\n[Splatt3R-PointNet++] Saved training summary to: {summary_path}")
