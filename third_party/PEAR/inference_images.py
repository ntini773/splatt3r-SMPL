...
def save_obj(mesh_path, vertices, faces):
...
def inference( config_name, devices, input_path=None, output_path = None):
    ...
    mesh_output_path = os.path.join(output_path, "meshes")
    ...
    mesh_faces = body_renderer.faces[0].detach().cpu().numpy()
    ...
    for idx, img_path in enumerate(image_paths):
        ...
        for bbox_id in range(num_bbox):
            ...
            with open("a.txt", "w") as f:
                for key, val in outputs['body_param'].items():
                    f.write(f"{key}:\n")
                    ...
                    try:
                        ...
                        f.write("shape: Not available\n")

                    # values (handle torch / numpy / others)
                    data = val
                    try:
                        # try batch index
                        data = val[0]
                    ...
                    f.write(f"values:\n{data}\n")
                    f.write("\n" + "="*50 + "\n\n")
            print(f"{outputs['body_param'].keys()}")
