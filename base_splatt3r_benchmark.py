...
class BaseSplatt3RBenchmarkDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        context = [self._load_context_view(seq, f'cam_{cam_idx:02d}', frame_name) for cam_idx in context_indices]
...
def _make_dataset(config):
    ...
    return BaseSplatt3RBenchmarkDataset(data, config.data.resolution)
...
def _evaluate_batch(model, batch, loss_weights, average_over_mask=True, background_weight=0.1):
    ...
    gt_normals = torch.stack([t['normal_map'] for t in batch['target']], dim=1).float().to(color.device)
...
def run_benchmark(config):
    ...
    save_dir = '/ssd_scratch/gnrs/base_splatt3r_eval_results'
    os.makedirs(save_dir, exist_ok=True)

    checkpoint_path = getattr(config, 'checkpoint_path', None)
    if checkpoint_path is None:
        checkpoint_path = getattr(config, 'splatt3r_checkpoint_path', None)
    if checkpoint_path is None:
        raise ValueError('Provide config.checkpoint_path or config.splatt3r_checkpoint_path for the base Splatt3R checkpoint.')

    print(f'Loading base Splatt3R checkpoint from {checkpoint_path} ...')
    model = splatt3r_main.MAST3RGaussians.load_from_checkpoint(checkpoint_path, map_location='cpu', strict=False)
    model.cuda().eval()
    ...
    for step, batch in enumerate(data_loader):
        ...
        print(f"[{step + 1:04d}/{len(data_loader):04d}] {sample_key}: "
    ...
    print('\nBase Splatt3R evaluation summary:')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Benchmark a base Splatt3R checkpoint on MVHumanNet test data.')
    parser.add_argument('config_path', help='Path to a YAML config used for data paths and evaluation settings.')
