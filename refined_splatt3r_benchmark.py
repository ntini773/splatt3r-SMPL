...
def _evaluate_batch(model, batch, loss_weights, average_over_mask=True, background_weight=0.1):
    ...
    # ── Standard rendering (no masking) ──────────────────────────────────────
    color, _ = model.decoder.forward(batch, (h, w))
    rendered_normals = model.decoder.render_normals(batch, (h, w))
    ...
    # Strict mask conversion for target views
    dataset_human_mask = torch.stack(
        [torch.as_tensor(t['mask_human'], device=device).bool() for t in batch['target']],
        dim=1,
    )   # (B, V, H, W)
...
def run_benchmark(config):
    ...
    aggregate = {'loss': [], 'mse': [], 'psnr': [], 'lpips': [], 'ssim': [], 'normal': [], 'gaussians_retained_pct': []}
    per_sample = {}
